...
fichier = File.open("map_andherra.txt", "w+") #Fichier directement dans le Drive
logs = File.open("logs.txt", "w+")
fichier.puts "Map scanner v1.0.1, par snwfdhmp. Derniere mise a jour du fichier : #{fichier.mtime}"
coograph_file = File.open("coographfile.txt", "w+");
nombre_bases = 0
puts "Scan en cours ..."
coordonnees = Array.new(1)
coograph = Array.new(1) #coordonnees a grapher
avancement = 0
while x < 311 do
agent.page.search('.map_td').each do |cell|
	if cell.at(".actions_map") != nil
		...
		#next if points.delete('.').delete(',').delete(' ').to_i < 500000
		...
		#next if alliance == "Heaven" #ALLIANCES NON REFERENCEES
		...
		logs.puts coograph[nombre_bases]
		nombre_bases += 1
	end
	end
end
