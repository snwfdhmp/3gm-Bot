#!/bin/env ruby
# Coded by snwfdhmp in 2014-2015 
# 
# => 3GM AUTONOMATE (Autonomic automate)
#  Ce programme fait parti d'une série de bots conçus 
#   pour le jeu 3gm.fr (jeu indépendant créé par Spoke).
#  Ils sont tous libres d'utilisation mais
#   développés par snwfdhmp.
#
# encoding: utf-8
require 'rubygems'
require 'nokogiri'
...
puts "Bienvenue #{pseudo} !" #Affichage d'un premier compte rendu de la base
...
while input!="logout"
	...
	case input 
		when "autobuild"
			...
			urls.each do |url|
				...
				agent.page.search('.demi_bloc').each do |bloc|
					...
					#On passe au batiment suivant si le niveau est déjà >= à celui désiré
					...
					if (dev == 1) #Si on peut améliorer le batiment => on le fait !!
		...
		when "home" #Donne un compte rendu de la base
			get_all_infos(agent, urls)

		when "map" #Scan la map

			x=100
			y=100

			agent.get("http://www.3gm.fr/game/map.php?x=#{x}&y=#{y}")#se rend sur l'url et demande la case (x;y)

			agent.page.search("#map_table").at(".map_tr").each do |ligne| #Puis scanne les 9*9 cases présentes sur la map affichée
				x-=10
